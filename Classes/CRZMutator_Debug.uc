...
class CRZMutator_Debug extends UTMutator config(MutatH0r);
...
var float LockedPhase;
var float TweakValue1, TweakValue2;
var bool Crouch;
...
simulated event Tick(float DeltaTime)
{
  ...
  foreach WorldInfo.AllPawns(class'UTPawn', P)
  {
    ...
    else if (P.bIsCrouched && !prevCrouch)
    {
      //P.EyeHeight -= extraHeadHeight;
      //P.OldZ += extraHeadHeight;
    }

    deltaZ = extraHeadHeight - adjustedActorPosition;
    P.Move(vect(0,0,1) * deltaZ);
...
function DumpInformation(UTPawn P)
{
	`log("Pawn: Location=" $ P.Location $ "; MeshTranslationOffset=" $ P.MeshTranslationOffset);
...
function Mutate(string MutateString, PlayerController Sender)
{
	...
	if (MutateString == "dump")
		DumpInformation(UTPawn(Sender.Pawn));
	else if (MutateString == "+tweak1")
		TweakValue1 += 1;
	else if (MutateString == "-tweak1")
		TweakValue1 -= 1;
	else if (MutateString == "+tweak2")
		TweakValue2 += 1;
	else if (MutateString == "-tweak2")
		TweakValue2 -= 1;
	...
	else if (MutateString == "detach")
	{
		...
		{
			if (PlayerController(P.Controller) == None)
        P.DetachFromController();
		}		
	}
	else if (MutateString == "upd")
		DummyMove();
  else if (left(MutateString, 6) == "phase ")
    LockedPhase = float(mid(mutatestring, 6));
	else
		Super.Mutate(MutateString, Sender);

	`log("TweakValue1=" $ TweakValue1 $ "; TweakValue2=" $ TweakValue2);
}
...
//	Slider.SetFloat("minimum", 0.5);
//	Slider.SetFloat("maximum", 4.0);
//	Slider.SetSnapInterval(0.5);
